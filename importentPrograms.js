...
let house = [8,5,6,3,2,9,11,5]
function maxSum(house) {
    var n = house.length
    if(n === 1){
        return house[0]
    }
    ...
    houseCount[0] = house[0]
    houseCount[1] = Math.max(house[0], house[1])
    console.log(houseCount)
    for (let i=2; i< n ; i++){
        houseCount[i] = Math.max(house[i]+houseCount[i-2], houseCount[i-1])
    }
    console.log(houseCount)
    return houseCount[n-1]
}

let result = maxSum(house)
...
// array = []  /* ans: array =[]*/
/*or*/
// array = array.splice(0, array.length)  /*ans : arrayCopy= []*/
/*or*/
// array.length = 0
/*or*/
...
console.log(typeof arrayList)    /*wrong way*/
console.log(Array.isArray(arrayList))  /*Correct way*/
...
let students = [{"name":"sai", "score":10},{"name":"krishna", "score":20},{"name":"ravi", "score":14},{"name":"harika", "score":22}]

let newData = students.filter((value) => {
    return value.score > 15
});
console.log(newData)

let datatwo = students.reduce((test, value) =>{
    if(value.score < 14){
        test.push(value)
    }
    return test
